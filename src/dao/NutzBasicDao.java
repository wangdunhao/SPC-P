...
public class NutzBasicDao{
	...
	private NutzBasicDao(){
		dao = new NutIoc(new JsonLoader("/config/dao.js")).get(Dao.class);//dao.js文件的放置的位置注意，链接数据库的
	}
	public static NutzBasicDao getInstance(){
		return uniqueInstance;
	}
	/*
	public Dao getDao() { // 暂不考虑线程同步的问题 D:/Java/workspace/nutzbook/conf/ioc/dao.js
	    if (ioc == null)
	        ioc = new NutIoc(new JsonLoader("/config/dao.js")); 
	...
	*/
	 ...
	 * @return
	 */
	public <T> List<T> searchall(Class<T> c,Condition condition){
	 ...
	 * 
	 * @param c			查询的表
	 * @param value		查询条目对应的值
	 * @param fieldName 查询条目
	 * @return list
	 */
	public <T> List<T> searchColumnLike(Class<T> c, int value,
			String fieldName ,String orderby) {
		Entity<T> entity = dao.getEntity(c);
		String column = entity.getField(fieldName).getColumnName();
		return dao.query(c, Cnd.where(column, "=", value).desc(orderby));
	...
	public <T> List<T> searchPageByLike(Class<T> c,String value, String orderby,int currentPage,int pageSize){
		...
		for(MappingField f:fields){
			if(!f.isId()){
				SqlExpression e = Cnd.exps(f.getColumnName(), "LIKE","%" + value + "%");
				if(group==null){
					group = Cnd.exps(e);
				}else{
					group.or(e);
	 ...
	 * @return List
	 */
	public <T> List<T> searchPageByLike(Class<T> c,String value,String fieldName,
			String value1, String orderby,int currentPage,int pageSize){
		Entity<T> entity = dao.getEntity(c);
	...
	public <T> List<T> searchByPageLike(Class<T> c,int value,String fieldName,int currentPage,int pageSize){
      Entity<T> entity = dao.getEntity(c);
	 ...
	 * @param currentPage 	当前页面
	 * @param pageSize		页面大小
	 * @param fieldName 	查询条件2
	 * @param value2		查询条件2 的值 如果不需要查询条件2 则直接用null
	 * @return int
	 */
	...
	public <T> int searchByPageLike(Class<T> c,String fieldName,String value){
		...
		return dao.count(c, Cnd.where(column, "=", value));
		
	}
	public <T> int searchByPageLike(Class<T> c,String fieldName,int value){
		Entity<T> entity = dao.getEntity(c);
	 ...
	 * @return
	 */
	public <T> List<T> findByCondition(Class<T> c,String columnname,int value){
		return dao.query(c, Cnd.where(columnname, "=", value));
	}
	 ...
	 * @param <T>
	 * @param t         插入数据库的对象
	 * @param fieldName 关联数据的字段名，一般为List对象，T中定义的对象名称
	 * @return T
	 */
	public <T> T saveWith(T t,String fieldName){
	...
	public void deletes(String tableName){
	   Sql sql= Sqls.create("delete from "+tableName+ " where 1=1");
	   dao.execute(sql);
